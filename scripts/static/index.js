...
var targetElement = document.getElementById("dropArea");
...
let file;
let selectedModelPath = 'models/Br35H_model_97.67.h5'; // Default model path
...
function handleCheckboxChange(event) {
  ...
  if (checkbox === switch1 && checkbox.checked) {
    ...
    switch2.checked = false;
    sendSwitchIndex(3);
  } else if (!switch1.checked && !switch2.checked && !switch3.checked) {
      // Make switch1 active as default
      switch1.checked = true;
      // If all switches are unchecked, call sendSwitchIndex(1)
      sendSwitchIndex(1);
    }
...
// Function which is sending info which switch is active to server
function sendSwitchIndex(selectedSwitchIndex) {
  fetch('/switchChange', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ selectedSwitchIndex }),
  })
    .then(response => response.json())
    .then(result => {
      console.log(result);
      selectedModelPath = result.message; // Update the selected model path
      // Add additional logic based on the server's response if needed
    })
    .catch(error => console.error('Error:', error));
}

// Add the event listeners to switches
if (switch1) {
...
dropArea.style.backgroundColor = 'rgb(246,246,247)';
...
document.getElementById('uploadForm').addEventListener('submit', function (event) {
    ...
    console.log("The form is submitted", file);
    ...
    if (file != undefined) {
        ...
        .then(result => {
            ...
            tempImage.onload = function () {
                ...
                if (predictedClass === 0) {
                    if (selectedModelPath === 'models/chest_xray_model_96.09.h5') {
                        outputMessage.innerText = 'Good news! This is not a pneumonia! I am saying it with ' + predictedProbability + ' confidence.';
                    } else {
                 ...
                 if (selectedModelPath === 'models/chest_xray_model_96.09.h5') {
                          outputMessage.innerText = 'Unfortunately, this looks like a pneumonia. I am saying it with ' + predictedProbability + ' confidence. Be strong and consult your specialist doctor regarding the following steps. You will win this fight, I truly believe in you.';

                    } else {
                             outputMessage.innerText = 'Unfortunately, this looks like a tumor. I am saying it with ' + predictedProbability + ' confidence. Be strong and consult your specialist doctor regarding the following steps. You will win this fight, I truly believe in you.';
        ...
        tempImage.onload = function () {
            ...
            outputMessage.innerText = 'Hey there! To get started, please upload an image.';
            showOutputBlock(outputBlock);
        };
        if (tempImage.complete) {
            tempImage.onload(); // Manually trigger the onload event
